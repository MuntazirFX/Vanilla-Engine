...
void CL_CreateCmd( void )
{
	...
	cl.data.iWeaponBits = cl.frame.client.weapons & 1;
...
boost::asio::awaitable<void> CL_CheckFrameSmooth()
{
	...
	if(now - last_frame_time > std::chrono::duration<double>(host.realframetime))
	{
		...
		co_await boost::asio::post(ioc_cl_main, boost::asio::use_awaitable);
	}
	co_return;
}
...
======================
*/
boost::asio::awaitable<void> CL_PrepSound( void )
{
	int	i, sndcount, step;
...
=================
*/
boost::asio::awaitable<void> CL_PrepVideo( void )
{
...
void CL_ConnectionlessPacket( netadr_t from, sizebuf_t *msg )
{
	...
	else if( !Q_strcmp( c, "errormsg" ))
	{
		...
		Cmd_ExecuteString( "menu_connectionprogress disconnect", src_command );
...
=================
*/
boost::asio::awaitable<void> Co_CL_Precache( void )
{
	int	spawncount;
	...
	//CL_PrepSound();
	//CL_PrepVideo();
	auto aw1 = CL_PrepSound();
	auto aw2 = CL_PrepVideo();
...
void CL_InitLocal( void )
{
	...
	cl_lightstyle_lerping = Cvar_Get( "cl_lightstyle_lerping", "1", CVAR_READ_ONLY, "enable animated light lerping (perfomance option)" );
	...
	ui_renderworld = Cvar_Get("ui_renderworld", "1", CVAR_ARCHIVE, "render world when UI is visible");
