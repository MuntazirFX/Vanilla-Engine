...
static void SPR_DrawGeneric( int frame, float x, float y, float width, float height, const wrect_t *prc )
{
	...
	ImGui_Surface_DrawImage( texnum, x, y, x + width, y + height, s1, t1, s2, t2,  clgame.ds.spriteColor[0], clgame.ds.spriteColor[1], clgame.ds.spriteColor[2], clgame.ds.spriteColor[3] );
#else
...
#ifdef XASH_VGUI2
   VGUI2_Surface_DrawStringLen(pText, &x, &y);
    x = clgame.scrInfo.iWidth / 2 - x / 2;
    y = clgame.scrInfo.iHeight / 4;
    VGUI2_Surface_DrawConsoleString(x, y, pText, colorDefault[0], colorDefault[1], colorDefault[2], colorDefault[3]);
#elif defined XASH_IMGUI
    ImGui_Surface_DrawStringLen(pText, &x, &y);
	x = scr_width->integer / 2 - x / 2;
	y = scr_height->integer / 4;
    ImGui_Surface_DrawConsoleString(x, y, pText, colorDefault[0], colorDefault[1], colorDefault[2], colorDefault[3]);
#else
...
void CL_DrawScreenFade( void )
{
	...
	else ImGui_Surface_SetRenderMode( kRenderTransTexture );
	ImGui_Surface_DrawRectangle(0, 0, scr_width->integer, scr_height->integer, sf->fader, sf->fadeg, sf->fadeb, iFadeAlpha);
#else
...
void CL_DrawCrosshair( void )
{
	...
	*(int *)clgame.ds.spriteColor = *(int *)crosshair_state.rgbaCrosshair;
	ImGui_Surface_EnableScissor( x - 0.5f * width, y - 0.5f * height, width, height );
...
#if defined(XASH_IMGUI) && !defined(XASH_VGUI2)
		ImGui_Surface_SetRenderMode( kRenderTransTexture );
		ImGui_Surface_DrawImage( cls.loadingBar, x, y, x + width, y + height, 0, 0, 1, 1, 128, 128, 128, 255 );
#else
...
#if defined(XASH_IMGUI) && !defined(XASH_VGUI2)
		ImGui_Surface_SetRenderMode( kRenderTransTexture );
		ImGui_Surface_DrawImage( cls.loadingBar, x, y, x + width, y + height, 0, 0, s2, 1, 208, 152, 0, 255 );
#else
...
#if defined(XASH_IMGUI) && !defined(XASH_VGUI2)
		ImGui_Surface_SetRenderMode( kRenderTransTexture );
		ImGui_Surface_DrawImage( cls.loadingBar, x, y, x + width, y + height, 0, 0, 1, 1, 255, 255, 255, 255 );
#else
		pglColor4ub( 255, 255, 255, 255 );
		GL_SetRenderMode( kRenderTransTexture );
		R_DrawStretchPic( x, y, width, height, 0, 0, 1, 1, cls.loadingBar );
...
#if defined(XASH_IMGUI) && !defined(XASH_VGUI2)
	ImGui_Surface_SetRenderMode( kRenderTransTexture );
	ImGui_Surface_DrawImage( cls.pauseIcon, x, y, x + width, y + height, 0, 0, 1, 1, 255, 255, 255, 255 );
#else
	pglColor4ub( 255, 255, 255, 255 );
	GL_SetRenderMode( kRenderTransTexture );
...
{
#if defined(XASH_IMGUI) && !defined(XASH_VGUI2)
	ImGui_Surface_SetRenderMode(kRenderNormal);
#endif
...
void CL_InitEdicts( void )
{
	...
	cls.packet_entities = (entity_state_t*)Z_AlignedAlloc( sizeof( entity_state_t ) * cls.num_client_entities, alignof(entity_state_t) );
	clgame.entities = (cl_entity_t*)Mem_AlignedAlloc( clgame.mempool, sizeof( cl_entity_t ) * clgame.maxEntities, alignof(cl_entity_t) );
	clgame.static_entities = (cl_entity_t*)Mem_AlignedAlloc( clgame.mempool, sizeof( cl_entity_t ) * MAX_STATIC_ENTITIES, alignof(cl_entity_t) );
	clgame.numStatics = 0;
	...
	if(( clgame.maxRemapInfos - 1 ) != clgame.maxEntities )
	{
		...
		clgame.remap_info = (remap_info_t **)Mem_ZeroAlloc( clgame.mempool, sizeof( remap_info_t* ) * clgame.maxRemapInfos );
	}
}
...
static client_sprite_t *GAME_EXPORT pfnSPR_GetList( const char *psz, int *piCount )
{
	...
	pList = (client_sprite_t*)Mem_ZeroAlloc( pool, sizeof( client_sprite_t ) * numSprites );
...
void GAME_EXPORT CL_FillRGBA( int x, int y, int width, int height, int r, int g, int b, int a )
{
	...
	ImGui_Surface_SetRenderMode(kRenderTransAdd);
	ImGui_Surface_DrawRectangle(x1, y1, w1, h1, r, g, b, a);
#else
	float x1 = x, y1 = y, w1 = width, h1 = height;
	r = bound( 0, r, 255 );
	g = bound( 0, g, 255 );
...
int GAME_EXPORT pfnDrawCharacter( int x, int y, int number, int r, int g, int b )
{
	...
	int a = max(r, max(g, b));
	rgba_t rgb = { (byte)r,(byte)g,(byte)b,(byte)a };
	int w = VGUI2_Surface_DrawChar(x, y, number, r, g, b, a);
	...
	rgba_t rgb = { (byte)r,(byte)g,(byte)b,(byte)a };
	ImGui_Surface_SetRenderMode( kRenderTransAdd );
	int w = ImGui_Surface_DrawChar(x, y, number, r, g, b, a);
	clgame.ds.adjust_size = false;
	return w;
#else
	number &= 255;
...
#ifdef XASH_VGUI2
    drawLen = VGUI2_Surface_DrawConsoleString(x, y, string, clgame.ds.textColor[0], clgame.ds.textColor[1], clgame.ds.textColor[2], clgame.ds.textColor[3]);
#elif defined XASH_IMGUI
	drawLen = ImGui_Surface_DrawConsoleString(x, y, string, clgame.ds.textColor[0], clgame.ds.textColor[1], clgame.ds.textColor[2], clgame.ds.textColor[3]);
#else
...
#elif defined XASH_IMGUI
	ImGui_Surface_DrawStringLen( pText, length, height );
#else
...
=============
*/
static void GAME_EXPORT pfnGetViewAngles( vec3_t_ref angles )
{
...
=============
*/
static void GAME_EXPORT pfnSetViewAngles( const vec3_t angles )
{
...
=============
*/
static void GAME_EXPORT pfnApplyShake( vec3_t_ref origin, vec3_t_ref angles, float factor )
{
...
=============
*/
static int GAME_EXPORT pfnPointContents( const vec3_t p, int *truecontents )
{
	int	cont, truecont;
...
=============
*/
static pmtrace_t *GAME_EXPORT pfnTraceLine( const vec3_t start, const vec3_t end, int flags, int usehull, int ignore_pe )
{
	static pmtrace_t	tr;
	int		old_usehull;
...
static void GAME_EXPORT pfnPlaySoundByNameAtLocation( const char *szSound, float volume, const vec3_t origin )
{
	int hSound = S_RegisterSound( szSound );
...
=============
*/
static void GAME_EXPORT pfnPlaySound( int ent, const vec3_t org, int chan, const char *samp, float vol, float attn, int flags, int pitch )
{
...
=============
*/
static void GAME_EXPORT pfnLocalPlayerViewheight( vec3_t_ref view_ofs )
{
...
=============
*/
static void GAME_EXPORT pfnLocalPlayerBounds( int hull, vec3_t_ref mins, vec3_t_ref maxs )
{
	if( hull >= 0 && hull < 4 )
	{
...
=============
*/
void GAME_EXPORT CL_PlayerTrace( const vec3_t start, const vec3_t end, int traceFlags, int ignore_pe, pmtrace_t *tr )
{
	if( !tr ) return;
...
=============
*/
void GAME_EXPORT CL_PlayerTraceExt( const vec3_t start, const vec3_t end, int traceFlags, int (*pfnIgnore)( physent_t *pe ), pmtrace_t *tr )
{
	if( !tr ) return;
...
=============
*/
static const char *GAME_EXPORT pfnTraceTexture( int ground, const vec3_t vstart, const vec3_t vend )
{
	physent_t *pe;
...
=============
*/
static struct msurface_s *GAME_EXPORT pfnTraceSurface( int ground, const vec3_t vstart, const vec3_t vend )
{
	physent_t *pe;
...
{
#if defined(XASH_IMGUI) && !defined(XASH_VGUI2)
	ImGui_Surface_SetCustomBlend( blendsrc, blenddst );
#else
	pglEnable( GL_BLEND );
...
#ifdef XASH_VGUI2
    clgame.ds.adjust_size = true;
    int w = VGUI2_Surface_DrawChar(x, y, number, g_color_table[7][0], g_color_table[7][1], g_color_table[7][2], g_color_table[7][3]);
    clgame.ds.adjust_size = false;
    return w;
#elif defined XASH_IMGUI
	clgame.ds.adjust_size = true;
	int w = ImGui_Surface_DrawChar(x, y, number, g_color_table[7][0], g_color_table[7][1], g_color_table[7][2], g_color_table[7][3]);
	clgame.ds.adjust_size = false;
	return w;
#else
	if( !cls.creditsFont.valid )
...
=============
*/
void GAME_EXPORT TriVertex3fv( const vec3_t v )
{
	pglVertex3fv( v.data() );
}
...
int GAME_EXPORT TriSpriteTexture( model_t *pSpriteModel, int frame )
{
	...
	if( gl_texturenum <= 0 || gl_texturenum > R_TextureNum() )
	{
...
=============
*/
int TriWorldToScreen( const vec3_t world, vec3_t_ref screen )
{
	int	retval;
...
=============
*/
void GAME_EXPORT TriFog( const vec3_t flFogColor, float flStart, float flEnd, int bOn )
{
	if( RI.fogEnabled || !gl_fog->integer ) return;
	//RI.fogCustom = true;
	...
	RI.fogStart = (flStart) ? flStart : 1.0f;
	RI.fogEnd = (flEnd || !RI.fogDensity) ? flEnd : 0.25f / RI.fogDensity;

	pglEnable(GL_FOG);
#if _WIN32 || TARGET_OS_OSX
...
#endif
	pglFogfv(GL_FOG_COLOR, RI.fogColor.data());
...
=============
*/
int GAME_EXPORT TriBoxInPVS( const vec3_t mins, const vec3_t maxs )
{
...
=============
*/
void GAME_EXPORT TriLightAtPoint( const vec3_t pos, float *value )
{
	color24	ambient;
...
static void GAME_EXPORT NetAPI_Status( net_status_t *status )
{
	...
	status->local_address = net_ipv4_local;
...
#ifdef XASH_VGUI2
	extern int VGui2_Initialize(cl_enginefunc_t * pEnginefuncs);
	//extern void VGui2_Startup();
    VGui2_Initialize(&gEngfuncs);
	//VGui2_Startup();
#endif

	Cvar_Get( "cl_nopred", "0", CVAR_ARCHIVE|CVAR_USERINFO, "disable client movement predicting" );
