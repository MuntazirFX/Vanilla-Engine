...
#ifndef XASH_VGUI2
	// dont hide game when connecting server
	if (!cls.changedemo)
...
{
    char *pszCodec = BF_ReadString( msg );
    int quality = BF_ReadByte( msg );
...
void CL_ParseVoiceData( sizebuf_t *msg )
{
    ...
    idx = BF_ReadByte( msg ) + 1;

    frames = BF_ReadByte( msg );

    size = BF_ReadShort( msg );
    size = min( size, 8192 );

    BF_ReadBytes( msg, received, size );
...
void CL_ParseServerMessage( sizebuf_t *msg )
{
	...
	while( 1 )
	{
		...
		case svc_customization:
			...
			break;
		case svc_weaponanim2: {
			param1 = BF_ReadByte(msg);	// iAnim
			param2 = BF_ReadByte(msg);	// body
			float framerate = (float)(BF_ReadShort(msg) / 256.0f);	// framerate
			CL_WeaponAnim2(param1, param2, framerate);
			break;
		}
