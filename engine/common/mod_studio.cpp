...
typedef int (*STUDIOAPI)( int, sv_blending_interface_t**, server_studio_api_t*,  matrix3x4 *transform, matrix3x4 (*bones)[MAXSTUDIOBONES] );
...
====================
*/
void Mod_AddToStudioCache( float frame, int sequence, const vec3_t angles, const vec3_t origin, const vec3_t size, byte *pcontroller, byte *pblending, model_t *model, hull_t *hull, int numhitboxes )
{
...
====================
*/
mstudiocache_t *Mod_CheckStudioCache( model_t *model, float frame, int sequence, const vec3_t angles, const vec3_t origin, const vec3_t size, byte *pcontroller, byte *pblending )
{
	mstudiocache_t	*pCache;
	int		i;
...
====================
*/
static void Mod_StudioCalcBoneQuaterion( int frame, float s, mstudiobone_t *pbone, mstudioanim_t *panim, float *adj, vec4_t_ref q )
{
	int		j, k;
	vec4_t		q1, q2;
...
void Mod_StudioBigEndian( model_t *model, byte *buffer )
{
	...
	for( j = 0; j < phdr->numseq; j++ )
	{
		...
		if( pseqdesc->seqgroup )
		{
			...
			if( paSequences == NULL )
			{
				paSequences = (cache_user_t *)Mem_ZeroAlloc( com_studiocache, MAXSTUDIOGROUPS * sizeof( cache_user_t ));
...
void Mod_StudioGetAttachment( const edict_t *e, int iAttachment, vec3_t_ref origin, vec3_t_ref angles )
{
	...
	if( sv_allow_studio_attachment_angles->integer )
	{
        ...
        forward = (origin - bonepos).Normalize();
...
void Mod_GetBonePosition( const edict_t *e, int iBone, vec3_t_ref origin, vec3_t_ref angles )
{
	...
	if( angles ) VectorAngles({ studio_bones[iBone][0][0], studio_bones[iBone][0][1], studio_bones[iBone][0][2] }, angles ); // bone forward to angles
}
...
====================
*/
void Mod_StudioAccumulateBoneVerts( vec3_t_ref mins1, vec3_t_ref maxs1, int *counter1, vec3_t_ref mins2, vec3_t_ref maxs2, int *counter2 )
{
	vec3_t	midpoint;
...
void Mod_StudioComputeBounds( void *buffer, vec3_t_ref mins, vec3_t_ref maxs )
{
	...
	{
		vec3_c *pverts = (vec3_c *)((byte *)pstudiohdr + m_pSubModel[i].vertindex);

		for( j = 0; j < m_pSubModel[i].numverts; j++ )
			Mod_StudioBoundVertex( vecmins1, vecmaxs1, &counter1, pverts[j] );
	}
