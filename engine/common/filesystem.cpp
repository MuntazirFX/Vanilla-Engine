...
static void listdirectory_AndroidAsset(stringlist_t *list, const char *path)
{
...
const char *FS_FixFileCase( const char *path )
{
    ...
    if(!strncmp(path, host.rodir, Q_strlen(host.rodir)))
    {
        Q_strlwr( path + Q_strlen(host.rodir), path2 );
        return va( "%s%s", host.rodir, path2 );
    }
#endif
...
pack_t *FS_LoadPackPAK( const char *packfile, int *error )
{
	...
	pack = (pack_t *)Mem_ZeroAlloc( fs_mempool, sizeof( pack_t ));
	Q_strncpy( pack->filename, packfile, sizeof( pack->filename ));
	pack->handle = packhandle;
	pack->numfiles = 0;
	pack->files = (packfile_t *)Mem_ZeroAlloc( fs_mempool, numpackfiles * sizeof( packfile_t ));
...
static qboolean FS_AddPack_Fullpath( const char *pakfile, qboolean *already_loaded, qboolean keep_plain_dirs, int flags )
{
	...
	if( pak )
	{
		if( keep_plain_dirs )
		{
			...
			if( !insertion_point )
			{
				search = (searchpath_t *)Mem_ZeroAlloc( fs_mempool, sizeof( searchpath_t ));
				search->pack = pak;
	...
	if( host.type != HOST_UNKNOWN )
	{
		...
		for( i = 0; i < dirs.numstrings; i++ )
		{
			...
			if( !SI.games[SI.numgames] )
				SI.games[SI.numgames] = (gameinfo_t *)Mem_AlignedAlloc( fs_mempool, sizeof( gameinfo_t ), alignof( gameinfo_t ));
...
static int FS_SysFileTime( const char *filename )
{
	...
	if(!strcmp(filename, "/android_asset"))
	{
		return 0;
	}
	if(!strncmp(filename, "/android_asset/", 15))
	{
		// TODO : android assets's time is not available
		return 0;
	}
#endif
	struct stat buf;
...
#if defined __ANDROID__
	// handle android assets
	if(!strncmp(filepath, "/android_asset/", 15))
	{
		AAssetManager *mgr = Android_GetAssetManager();
		AAsset *asset = AAssetManager_open(mgr, filepath + 15, AASSET_MODE_RANDOM);

		if( asset == NULL )
		{
			const char *ffilepath = FS_FixFileCase( filepath );
			if( ffilepath != filepath )
				asset = AAssetManager_open(mgr, ffilepath + 15, AASSET_MODE_RANDOM);
		}
		...
		/*
#ifdef XASH_64BIT
		file->handle = AAsset_openFileDescriptor64(asset, &file->offset, &file->real_length);
#else
		...
		*/
		...
		file->asset = asset;
#ifdef XASH_64BIT
		file->real_length = AAsset_seek64( file->asset, 0, SEEK_END );
#else
		file->real_length = AAsset_seek( file->asset, 0, SEEK_END );
#endif
		...
		else
		{
			file->position = 0;
#ifdef XASH_64BIT
			AAsset_seek64(file->asset, 0, SEEK_SET);
#else
...
qboolean FS_SysFileExists( const char *path, qboolean caseinsensitive )
{
	...
	{
		AAssetManager *mgr = Android_GetAssetManager();
		AAsset *asset = AAssetManager_open(mgr, path + 15, AASSET_MODE_STREAMING);

        // speedup custom path search
        if( caseinsensitive && asset == NULL )
        {
            const char *fpath = FS_FixFileCase( path );
            if( fpath != path )
                asset = AAssetManager_open(mgr, fpath + 15, AASSET_MODE_STREAMING);
        }
...
qboolean FS_SysFolderExists( const char *path )
{
	...
	{
		AAssetManager *mgr = Android_GetAssetManager();
		AAssetDir *assetDir = AAssetManager_openDir(mgr, path + 15);
		const char *d_name = AAssetDir_getNextFileName( assetDir );
		AAssetDir_close(assetDir);
		return d_name != NULL;
	}
#endif
...
file_t *FS_OpenReadFile( const char *filename, const char *mode, qboolean gamedironly )
{
    ...
    if( search == NULL )
    {
        const char *ffilepath = FS_FixFileCase( filename );
        if( ffilepath != filename )
        {
            search = FS_FindFile( ffilepath, &pack_ind, gamedironly );
            filename = ffilepath;
        }
    }
#endif
...
file_t *FS_Open( const char *filepath, const char *mode, qboolean gamedironly )
{
    ...
    {
        const char *ffilepath = FS_FixFileCase( filepath );
        if( FS_CheckNastyPath( ffilepath, false ))
            return NULL;
        filepath = ffilepath;
    }
#endif
...
fs_offset_t FS_Read( file_t *file, void *buffer, size_t buffersize )
{
	...
	if( file->buff_ind < file->buff_len )
	{
        ...
        Mem_VirtualCopy( buffer, &file->buff[file->buff_ind], done );
		file->buff_ind += done;
	...
	if( buffersize > sizeof( file->buff ) / 2 )
	{
		...
		{
#ifdef XASH_64BIT
			AAsset_seek64( file->asset, file->offset + file->position, SEEK_SET );
#else
			AAsset_seek( file->asset, file->offset + file->position, SEEK_SET );
#endif
			nb = AAsset_read (file->asset, &((byte *)buffer)[done], count );
		}
		else
#endif
		{
	...
	else
	{
		...
		if(file->asset)
		{
			...
			//AAsset_seek( file->asset, file->offset + file->position, SEEK_SET );
#endif
			//nb = AAsset_read (file->asset, file->buff, count );
			const char *internal_buffer = (const char *)AAsset_getBuffer(file->asset);
            nb = count;
			Mem_VirtualCopy( file->buff, internal_buffer + file->offset + file->position, count );
		}
		else
#endif
		{
		...
		if( nb > 0 )
		{
            ...
            Mem_VirtualCopy( &((byte *)buffer)[done], file->buff, count );
			file->buff_ind = count;
			done += count;
		}
	}
...
int FS_Seek( file_t *file, fs_offset_t offset, int whence )
{
	...
	{
#ifdef XASH_64BIT
		if( AAsset_seek64( file->asset, file->offset + offset, SEEK_SET ) == -1 )
#else
		if( AAsset_seek( file->asset, file->offset + offset, SEEK_SET ) == -1 )
#endif
			return -1;
	}
	else
...
dll_user_t *FS_FindLibrary( const char *dllname, qboolean directpath )
{
	...
	hInst = (dll_user_t*)Mem_ZeroAlloc( host.mempool, sizeof( dll_user_t ));
...
static qboolean W_ReadLumpTable( wfile_t *wad )
{
	...
	if( FS_Read( wad->wadfile, srclumps, lat_size ) != lat_size )
	{
...
byte *W_ReadLump( wfile_t *wad, dlumpinfo_t *lump, fs_offset_t *lumpsizeptr )
{
	...
	size = FS_Read( wad->wadfile, buf, lump->disksize );
...
{
	dwadinfo_t	header;
	wfile_t		*wad = (wfile_t *)Mem_ZeroAlloc( fs_mempool, sizeof( wfile_t ));
	const char	*comment = "Generated by Xash WadLib. ";

	wad->wadfile = FS_SysOpen( filename, mode );
	...
	if( !wad->wadfile )
	{
		...
		{
			wad->wadfile = FS_SysOpen( ffilename, mode );
            filename = ffilename;
		}
	}
#endif
	...
	wad->mempool = Mem_AllocSubPool( fs_mempool, filename );
	...
	if( mode[0] == 'w' )
	{
		...
		FS_Write( wad->wadfile, &hdr, sizeof( hdr ) );
		FS_Write( wad->wadfile, comment, Q_strlen( comment ) + 1 );
		wad->infotableofs = FS_Tell( wad->wadfile );
	}
	else if( mode[0] == 'r' || mode[0] == 'a' )
	{
		...
		if( wad->mode == 'a')
		{
			...
			if( FS_Read( wad->wadfile, wad->lumps, lat_size ) != lat_size )
			{
				MsgDev( D_ERROR, "W_ReadLumpTable: %s has corrupted lump allocation table\n", wad->filename );
				W_Close( wad );
				return NULL;
			...
			FS_Seek( wad->wadfile, wad->infotableofs, SEEK_SET );
		}
		else
		{
...
void W_Close( wfile_t *wad )
{
	...
	if( wad->wadfile && ( wad->mode == 'a' || wad->mode == 'w'))
	{
		...
		// write the lumpinfo
		ofs = FS_Tell( wad->wadfile );
		FS_Write( wad->wadfile, wad->lumps, wad->numlumps * sizeof( dlumpinfo_t ));
	...
	if( wad->wadfile ) FS_Close( wad->wadfile );
...
{
#ifdef XASH_MEMDBG
	return _Mem_ZeroAlloc( fs_mempool, size, filename, fileline );
#else
	return Mem_ZeroAlloc( fs_mempool, size );
#endif
}
