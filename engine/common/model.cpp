...
{
#ifndef XASH_DEDICATED
	// clear lru texture data before clear models!!
	xe::TexLru_Clear();
#endif
...
static void Mod_LoadSubmodels( const dlump_t *l )
{
	...
	// allocate extradata
	out = (dmodel_t*)Mem_ZeroAlloc( loadmodel->mempool, count * sizeof( *out ));
	...
	if( world.loading )
		world.draw_surfaces = (msurface_t * *)Mem_ZeroAlloc( loadmodel->mempool, world.max_surfaces * sizeof( msurface_t* ));
}
...
static void Mod_LoadTextures( const dlump_t *l )
{
	...
	loadmodel->textures = (texture_t **)Mem_ZeroAlloc( loadmodel->mempool, loadmodel->numtextures * sizeof( texture_t* ));
...
static void Mod_LoadTexInfo( const dlump_t *l )
{
	...
	count = l->filelen / sizeof( *in );
	out = (mtexinfo_t *)Mem_ZeroAlloc( loadmodel->mempool, count * sizeof( *out ));
...
=================
*/
static void Mod_BuildPolygon( mextrasurf_t *info, msurface_t *surf, int numVerts, const vec3_t *verts )
{
	float		s, t;
	uint		bufSize;
	...
	buffer = (byte *)Mem_ZeroAlloc( loadmodel->mempool, bufSize );
...
=================
*/
static void Mod_SubdividePolygon( mextrasurf_t *info, msurface_t *surf, int numVerts, vec3_t *verts, float tessSize )
{
	vec3_t		vTotal, nTotal, tTotal, bTotal;
	vec3_t		front[MAX_SIDE_VERTS], back[MAX_SIDE_VERTS];
	float		m, oneDivVerts, dist, dists[MAX_SIDE_VERTS];
	...
	for( i = 0; i < 3; i++ )
	{
		...
		for( f = j = b = 0; j < numVerts; j++ )
		{
			...
			if(( dists[j] > 0.0f ) != ( dists[j+1] > 0.0f ))
			{
                ...
                front[f] = back[b] = fma((verts[j+1] - verts[j]), dist, verts[j]);
				f++, b++;
			}
		}

		Mod_SubdividePolygon( info, surf, f, front, tessSize );
		Mod_SubdividePolygon( info, surf, b, back, tessSize );
		return;
	}
	...
	for( i = 0; i < numVerts; i++ )
	{
		...
		// vertex
		VectorCopy( verts[i], out->vertex );
		VectorCopy( normal, out->normal );
		VectorCopy( tangent, out->tangent );
		VectorCopy( binormal, out->binormal );

		VectorAdd( vTotal, verts[i], vTotal );
		...
		{
			// texture coordinates
			s = DotProduct( verts[i], texinfo->vecs[0] ) + texinfo->vecs[0][3];
			s /= texinfo->texture->width;

			t = DotProduct( verts[i], texinfo->vecs[1] ) + texinfo->vecs[1][3];
			t /= texinfo->texture->height;
		}
		else
		{
			// texture coordinates
			s = DotProduct( verts[i], texinfo->vecs[0] ) * scale;
			t = DotProduct( verts[i], texinfo->vecs[1] ) * scale;
		}
		...
		{
			// lightmap texture coordinates
			s = DotProduct( verts[i], texinfo->vecs[0] ) + texinfo->vecs[0][3] - surf->texturemins[0];
			s += surf->light_s * LM_SAMPLE_SIZE;
			s += LM_SAMPLE_SIZE >> 1;
			s /= BLOCK_SIZE * LM_SAMPLE_SIZE;

			t = DotProduct( verts[i], texinfo->vecs[1] ) + texinfo->vecs[1][3] - surf->texturemins[1];
			t += surf->light_t * LM_SAMPLE_SIZE;
			t += LM_SAMPLE_SIZE >> 1;
			t /= BLOCK_SIZE * LM_SAMPLE_SIZE;
		}
...
void Mod_BuildSurfacePolygons( msurface_t *surf, mextrasurf_t *info )
{
	...
	{
		Mod_SubdividePolygon( info, surf, surf->numedges, verts, 64.0f );
		Mod_ConvertSurface( info, surf );
	}
	else
	{
		Mod_BuildPolygon( info, surf, surf->numedges, verts );
	}
...
void Mod_TesselatePolygon( msurface_t *surf, model_t *mod, float tessSize )
{
	...
	Mod_SubdividePolygon( info, surf, surf->numedges, verts, tessSize );
	Mod_ConvertSurface( info, surf );
...
static void Mod_LoadSurfaces( const dlump_t *l )
{
	...
	loadmodel->surfaces = (msurface_t*)Mem_ZeroAlloc( loadmodel->mempool, count * sizeof( msurface_t ));
	loadmodel->cache.data = (mextrasurf_t*)Mem_AlignedAlloc( loadmodel->mempool, count * sizeof( mextrasurf_t ), alignof(mextrasurf_t));
...
static void Mod_LoadVertexes( const dlump_t *l )
{
	...
	out = loadmodel->vertexes = (mvertex_t*)Mem_ZeroAlloc( loadmodel->mempool, count * sizeof( mvertex_t ));
...
static void Mod_LoadEdges( const dlump_t *l )
{
	...
	count = l->filelen / sizeof( *in );
	loadmodel->edges = out = (medge_t*)Mem_ZeroAlloc( loadmodel->mempool, count * sizeof( medge_t ));
...
static void Mod_LoadMarkSurfaces( const dlump_t *l )
{
	...
	count = l->filelen / sizeof( *in );
	loadmodel->marksurfaces = out = (msurface_t **)Mem_ZeroAlloc( loadmodel->mempool, count * sizeof( *out ));
...
static void Mod_LoadNodes( const dlump_t *l )
{
	...
	out = loadmodel->nodes = (mnode_t *)Mem_ZeroAlloc( loadmodel->mempool, loadmodel->numnodes * sizeof( *out ));
...
static void Mod_LoadLeafs( const dlump_t *l )
{
	...
	out = (mleaf_t *)Mem_ZeroAlloc( loadmodel->mempool, count * sizeof( *out ));
...
static void Mod_LoadPlanes( const dlump_t *l )
{
	...
	out = (mplane_t *)Mem_ZeroAlloc( loadmodel->mempool, count * sizeof( *out ));
...
static void Mod_LoadEntities( const dlump_t *l )
{
	...
	loadmodel->entities = (char*)Mem_ZeroAlloc( loadmodel->mempool, l->filelen + 1 );
	Mem_VirtualCopy( loadmodel->entities, mod_base + l->fileofs, l->filelen );
	if( !world.loading ) return;
...
=================
*/
static void Mod_FindModelOrigin( const char *entities, const char *modelname, vec3_t_ref origin )
{
	char	*pfile;
	string	keyname;
	char	token[2048];
...
//			mod->hulls[j].lastclipnode = mod->numclipnodes - 1;
		...
		if( i != 0 )
		{
			...
			vec3_t origin(bm->origin[0], bm->origin[1], bm->origin[2]);
			Mod_FindModelOrigin( ents, va( "*%i", i ), origin );

			// flag 2 is indicated model with origin brush!
			if( !VectorIsNull( origin )) mod->flags |= MODEL_HAS_ORIGIN;
		}
...
model_t *Mod_FindName( const char *filename, qboolean create )
{
	...
	Q_strncpy( name, filename, MAX_MODEL_NAME);
	COM_FixSlashes( name );

	char* s = name;
	// ����ģ�͵�ʱ���ȷ������ͼǰ׺(@frogvari_) -> (models/@frogvari_v_zombibomb.mdl)���ҵ���һ��'@'Ϊ��ʼ�͵�һ��'_'Ϊ��β����Ϊ��ͼǰ׺
	int start = -1, end = 0, pos = 0;
	while (*s != '\0')
	{
		if (*s == '@')
		{
	...
	char suffix[MAX_MODEL_SUFFIX], origin_name[MAX_MODEL_NAME];
	if (end)
	{
		memset(suffix, 0, MAX_MODEL_SUFFIX);
		memset(origin_name, 0, MAX_MODEL_SUFFIX);
		Q_strncpy(suffix, name + start, min(MAX_MODEL_SUFFIX, end - start + 1));
		Q_strncpy(origin_name, name, start);
		Q_strncat(origin_name, name + start + Q_strlen(suffix), MAX_MODEL_NAME);
	}
	...
	for( i = 0, mod = cm_models; i < cm_nummodels; i++, mod++ )
	{
		...
		if (!mod->suffix[0]) continue;
		if (end != 0 && !Q_stricmp(mod->suffix, suffix) && !Q_stricmp(origin_name, mod->name))
		{
			// prolonge registration
			mod->needload = world.load_sequence;
			return mod;
	...
	if( i == cm_nummodels )
	{
		...
		{
			Cmd_ExecuteString("dumpprecache\n", src_command);
	...
	if (end)
	{
		Q_strncpy(mod->name, origin_name, MAX_MODEL_NAME);
		Q_strncpy(mod->suffix, suffix, MAX_MODEL_SUFFIX);
	}
	else
	{
...
void *GAME_EXPORT Mod_Calloc( int number, size_t size )
{
	...
	cu = (cache_user_t *)Mem_ZeroAlloc( com_studiocache, sizeof( cache_user_t ) + number * size );
