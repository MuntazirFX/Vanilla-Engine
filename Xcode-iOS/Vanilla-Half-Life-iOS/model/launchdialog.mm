...
const char *IOS_GetBundleDir()
{
	...
	strcpy(c_path, [[path stringByAppendingPathComponent:@"/Half-Life"] UTF8String]);
...
BOOL IOS_IsResourcesReady()
{
	...
	return ([fileManager fileExistsAtPath:[doc stringByAppendingPathComponent:@"valve"]] ||
            [fileManager fileExistsAtPath:[bundle stringByAppendingPathComponent:@"valve"]]);
}
...
void IOS_SetDefaultArgs()
{
	...
	static char *args[64] = { "xash", "-dev", "5", "-log", "-game", "valve", "-width", width_str, "-height", height_str };
...
void IOS_LaunchDialog( void )
{
	...
	int version = Q_buildnum();
	config.version = [NSString stringWithFormat:@"%d", version];
	config.reportLogLevel = BuglyLogLevelWarn;
	[Bugly startWithAppId:@"5f4cd68a0b" config:config];
...
void IOS_GetSystemVersion(int *major, int *minor, int *patch)
{
	...
	if(minor)
		*major = ver.minorVersion;
	if(patch)
		*major = ver.patchVersion;
}
